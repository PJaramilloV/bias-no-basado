...
-   ¿Existirán grupos de personas que compartan la misma tasa de reincidencia que la de las personas afroamericanas?

Si se puede encontrar otro grupo, entonces se podría usar para tratar de predecir la puntuación de COMPAS, para asi analizar si este comete discriminación basada en etnia o realmente es debido a este factor.

-   ¿Es posible predecir la etnia de una persona en base a su decil y raw_score?

Saber esto es clave para poder concluir que efectivamente existe una discriminación de parte COMPAS, puesto que existe una relación lo suficientemente evidente como para que un modelo pueda predecir la puntuación que le dio a una persona, basándose principalmente en la etnia.

-   ¿Es posible predecir el valor `raw_data` de la tabla `compas` de una persona en función de variables correlacionadas a la etnia?

En el análisis vimos que existían otras variables que hacían al grupo de los afroamericanos muy distintivo entre el resto, como la tasa de reincidencia y la edad. Encontrar estas relaciones inesperadas, ayudará a poder formular un modelo que no sea discriminatorio en su base. 

-   ¿Será posible enmascarar u omitir la etnia de la persona y conseguir resultados de COMPAS similares?

Esta es, por lo tanto, la pregunta principal del proyecto, ya que si es posible crear un modelo que nos permita predecir la puntuación que COMPAS le dio a una persona, basándonos exclusivamente en la edad, género y etnia de esta, entonces no sólo se evidenciaría que COMPAS no es discriminatorio, sino que también podría ser una tentadora metodología para crear modelos que tampoco lo sean.
